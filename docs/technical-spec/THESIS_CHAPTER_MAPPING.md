...
|---|---|---|---|
| RQ1: unified subject and delegation | A policy request can distinguish authenticated human/service identity, AI agent context and a constrained 1-hop delegation. | Standard protobuf contract, tenant-bound JWT validation, full-tuple proof and migrated mapper verified across real Odoo/gRPC/PostgreSQL transactions, including two-session retry. | Verify the same boundary with mTLS. |
| RQ2: runtime and memory | The evaluator remains sub-microsecond with zero allocations for stated in-memory cases. | 2026-09-12 local three-sample evaluator result: 390.3–492.8 ns/op, 0 B/op, 0 allocs/op. | Release-commit environment capture; dense candidate/collision cases; full data-plane latency percentiles. |
| RQ3: guardrails | Policy-defined typed obligations can route sensitive AI tool calls to deterministic approval or deny paths. | DSL/engine/API tests plus real Odoo non-rollback approval and replay/concurrency evidence. | Runtime mTLS and durable-revocation security evidence. |
| RQ4: empirical ERP evaluation | The proposed system is correct, secure and performant for a representative Odoo P2P workflow. | Go tests plus seven real Odoo/PostgreSQL/gRPC transaction cases and two-session retry. | Comparative experiment, mTLS and publication-quality production-path results. |
...
| 4. Odoo implementation | Repository addon design and verified transaction/concurrency results: generated client, JWT, full-tuple proof, nonce ledger and typed-obligation mapping. | Calling mTLS/multi-replica behavior verified before those separate gates pass. |
