...
>
> **Release-evidence update (2026-09-18):** GitHub-hosted CI has passed its race, coverage, Odoo and combined Control Plane/PDP E2E gates. Runtime images are hardened and digest-pinned. Benchmark documentation records separate core, local gRPC application-path and ERP purchase-confirmation boundaries. External append-only audit retention and deletion evidence remain the sole unchecked release gate.
>
...
- **Partially verified experiment**: the Go core benchmarks are valid for their scope; the real Odoo comparison covers one narrow warm purchase-confirmation workload only and cannot support a general ERP speedup claim.
...
| Distributed consistency | 8/10 | COW/event/reconcile correctness plus durable snapshot-first revocation propagation pass locally and in remote CI |
...
| Tests | 8/10 | Negative, PostgreSQL, protobuf, mTLS, seven-case Odoo, combined Control Plane E2E and remote race/coverage gates pass |
| Deployment | 8/10 | Repository-local images build from digest-pinned bases, are runtime hardened, and required E2E profiles pass in remote CI |
| Odoo integration | 9/10 | Fresh install, seven real ORM/mTLS-gRPC/PostgreSQL cases and two-session retry pass |
| Production readiness | 8/10 | All implemented release gates pass; external append-only retention/deletion evidence still blocks release |
...
| `git diff --check` | PASS |
| Race detector locally | NOT VERIFIED; local Windows toolchain has no GCC for CGO; GitHub-hosted Ubuntu CI passes |
...
| Dynamic delegator-status/daily-limit attenuation | NOT VERIFIED | `delegator_status` and `delegator_limit` are not supplied to or evaluated by the current Odoo/PDP path. The thesis may claim TTL, active-grant, proof, policy, SoD and revocation controls, but not immediate authority collapse from HR status or daily-limit changes. |
...
No previously identified P0 implementation finding remains open. Release is blocked by the external archive/rehearsal gate in the readiness checklist.
...
- **WORM** is reserved for an externally controlled append-only retention system with deletion evidence; the current PostgreSQL audit path is encrypted and durable, but is not WORM.
- **Cluster-wide** is reserved for a specified multi-replica test boundary, not a universal deployment claim.
...
- Race tests cannot be reproduced locally without a C compiler; the GitHub-hosted Ubuntu CI race job is the recorded remote evidence.
